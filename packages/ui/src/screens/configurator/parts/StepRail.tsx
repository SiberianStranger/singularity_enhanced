...
import { STEP_IDS, type StepId, type StepState, stepState } from "../steps.js";
  ...
  return (
    <li>
      ...
      >
        ...
        <span
          aria-hidden
          className={`shrink-0 font-mono text-sm ${active ? "" : MARK_TONE[state]}`}
        >
          {MARK[state]}
        </span>
    ...
    <nav aria-label={t("config.steps")} className="min-h-0 overflow-auto border-e border-line">
      <ol className="flex flex-col py-1">
        {STEP_IDS.map((id, index) => (
          <RailEntry key={id} step={id} active={index === step} onSelect={() => goToStep(index)} />
        ))}
      </ol>
    </nav>
  );
